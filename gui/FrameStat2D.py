...
from bin.Dialogs import DimDialog, NodalForce, ElementForce, DistributedLoad, \
                    TemperatureLoad, SupportDisplacement, StructuralDefect,\
                    ViewOptions, Materials, Sections, Support, EditElement, \
                    F1, F2, F3, F4, F5, Analysis_Options
...
#=============================================================================         
    ...
    def on_options(self,event):
        ...
        self.n_modes = len(self.NOD)*3 - suma            
...
#         self.hbox.Add(self.vbox2, 0, flag = wx.ALIGN_LEFT | wx.TOP)
    ...
    def on_tool_7(self,event):
        tb = event.GetEventObject()            
        if tb.GetToolState(self.IDs[6]) == True:
            dlg = F2([0, 0, 0, 0, 0],self.PATH)
            dlg.CenterOnScreen()
            val = dlg.ShowModal()
            if val == wx.ID_OK:
                self.Set_State(6)
                self.F2 = dlg.dat_out
                self.Local = dlg.Local
            else:
                pass
            dlg.Destroy()
            self.uncheck_tools(tb,self.IDs[6])
        else:
    ...
    def on_tool_8(self,event):
        tb = event.GetEventObject()            
        if tb.GetToolState(self.IDs[7]) == True:
            dlg = F3([0, 0, 0, 0],self.PATH)
            dlg.CenterOnScreen()
            val = dlg.ShowModal()
            if val == wx.ID_OK:
                self.Set_State(7)
                ...
                pass
            dlg.Destroy()
            self.uncheck_tools(tb,self.IDs[7])
        else:
    ...
    def on_tool_9(self,event):
        tb = event.GetEventObject()            
        if tb.GetToolState(self.IDs[8]) == True:
            dlg = F4([0, 0, 0, 0],self.PATH)
            dlg.CenterOnScreen()
            val = dlg.ShowModal()
            if val == wx.ID_OK:
                self.Set_State(8)
                ...
                pass
            dlg.Destroy()
            self.uncheck_tools(tb,self.IDs[8])
        else:
    ...
    def on_tool_10(self,event):
        tb = event.GetEventObject()            
        if tb.GetToolState(self.IDs[9]) == True:
            dlg = F5([0, 0, 0, 0],self.PATH)
            dlg.CenterOnScreen()
            val = dlg.ShowModal()
            if val == wx.ID_OK:
                self.Set_State(9)
                ...
                pass
            dlg.Destroy()
            self.uncheck_tools(tb,self.IDs[9])
        else:
    ...
    def MakePaneContent(self, pane):
        ...
        self.nb.AddPage(self.BCTab, "Fixities")
    ...
    def on_canvas_click(self, event):
        ...
        if self.case == 1:
            ...
            if dist < 0.05*self.Dim_X:
                ...
                yp = self.NOD[idx][2]
                node_ID = self.get_node_ID(xp,yp)
                self.append_F1(node_ID,self.F1)
                if hasattr(self, 'temp_F1'):
                    self.temp_F1.remove()
                    del self.temp_F1
                if hasattr(self, 'temp_F1M'):
...
#             self.case = 2
    ...
    def append_support(self,idx,triple,angle):
        ...
        self.gridBC.SetCellValue(self.n_supports-1,4,str(angle))
...
#         ND = self.axes.scatter(xy[0], xy[1], color = self.col1, s=5)
    ...
    def Properties(self, parent):
        ...
        self.gridSections = wx.grid.Grid(self.PropertieTab, size=(470,160))
        ...
        self.gridSections.CreateGrid(1,4)
        ...
        self.gridSections.SetColSize(3, 100)
        ...
        self.gridSections.SetCellValue(0,3,str(int(self.SEC[0][4])))
        ...
        self.gridMaterials = wx.grid.Grid(self.PropertieTab, size=(370,160))
        ...
        self.gridMaterials.CreateGrid(1,3)
...
#         self.vbox2S.Add(self.hbox1, 0, border=3, flag=wx.ALIGN_RIGHT | wx.ALL | wx.ALIGN_CENTER_VERTICAL)
    ...
    def Geometry(self, parent):
        ...
        self.gridElements = wx.grid.Grid(self.GeometryTab, size=(370,160))
    ...
    def BCs(self, parent):
        ...
        self.gridBC = wx.grid.Grid(self.BCTab, size=(343,300))
...
#                 self.BC[i][0] = self.BC[i][0] - 1
        ...
        for i in range(0,self.n_supports):
            ...
            if self.BC[i][0] != 0:
                if self.BC[i][1] == 1 and self.BC[i][2] == 1 and self.BC[i][3] == 1:
                    ...
                    patch = self.get_patch_hinge(p,0.03*self.Dim_X,self.BC[i][4],selcol)
                    self.axes.add_patch(patch)
                elif self.BC[i][1] == 0 and self.BC[i][2] == 1 and self.BC[i][3] == 0:
                    patch = self.get_patch_roller(p,0.03*self.Dim_X,self.BC[i][4],selcol)
                    self.axes.add_patch(patch)
                elif self.BC[i][1] == 1 and self.BC[i][2] == 0 and self.BC[i][3] == 0:
                    patch = self.get_patch_roller(p,0.03*self.Dim_X,90+self.BC[i][4],selcol)
                    self.axes.add_patch(patch)
         
        if self.show_F1 == True and len(self.FOR1) != 0:
            for i in range(0,len(self.FOR1)):
                if self.FOR1[i][0] != 0:
    ...
    def Initialization(self):
        ...
        self.MAT = [[1, 7850, 21000000, 1e-05]]
        self.SEC = [[1, 0.12, 0.0016, 0.45, 1]]
#         self.MAT = []
#         self.SEC = []
    ...
    def on_open(self, event):
        if event != 0:
            dlg = wx.FileDialog(self, message="Choose a file",defaultDir=os.getcwd()+'/examples', \
            ...
            Dir=os.getcwd()
            defaultPath = Dir + '/examples/New.txt' 
        ...
        forces_n = []
        for i in range(forn+1,fori):
            forces_n.append(list(map(float, lines[i].split())))
            
        forces_i = []
        for i in range(fori+1,ford):
            forces_i.append(list(map(float, lines[i].split())))
            
        forces_d = []
        for i in range(ford+1,fort):
            forces_d.append(list(map(float, lines[i].split())))
            
        forces_t = []
        for i in range(fort+1,fors):
            forces_t.append(list(map(float, lines[i].split())))
            
        forces_s = []
        for i in range(fors+1,forf):
            forces_s.append(list(map(float, lines[i].split())))
            
        forces_f = []
        for i in range(forf+1,para):
            forces_f.append(list(map(float, lines[i].split())))

        params = list(map(float, lines[para+1].split()))
...
#                 app.frame_Warning.Show()
    ...
    def Create_Input_File(self,filename):
        ...
        f.write('*FOR_I\n')
        if len(self.FOR2) != 0:
            if (self.FOR2[0][0] == 0 and len(self.FOR2) == 1) or len(self.FOR2) == 0:
                pass
            else:
                for i in range(0,len(self.FOR2)):
                    f.write(str(int(self.FOR2[i][0])) + '  ' )
                    for j in range(1,5):
                        f.write(str(float(self.FOR2[i][j])) + '  ')    
                    f.write('\n')
            
        f.write('*FOR_D\n')
        if len(self.FOR3) != 0:
            if (self.FOR3[0][0] == 0 and len(self.FOR3) == 1) or len(self.FOR3) == 0:
                pass
            else:
                for i in range(0,len(self.FOR3)):
                    f.write(str(int(self.FOR3[i][0])) + '  ' )
                    for j in range(1,4):
                        f.write(str(float(self.FOR3[i][j])) + '  ')    
                    f.write('\n')

        f.write('*FOR_T\n')
        if len(self.FOR4) != 0:
            if (self.FOR4[0][0] == 0 and len(self.FOR4) == 1) or len(self.FOR4) == 0:
                pass
            else:
                for i in range(0,len(self.FOR4)):
                    f.write(str(int(self.FOR4[i][0])) + '  ' )
                    for j in range(1,4):
                        f.write(str(float(self.FOR4[i][j])) + '  ')    
                    f.write('\n')

        f.write('*FOR_S\n')
        if len(self.FOR5) != 0:
            if (self.FOR5[0][0] == 0 and len(self.FOR5) == 1) or len(self.FOR5) == 0:
                pass
            else:
                for i in range(0,len(self.FOR5)):
                    f.write(str(int(self.FOR5[i][0])) + '  ' )
                    for j in range(1,4):
                        f.write(str(float(self.FOR5[i][j])) + '  ')    
                    f.write('\n')

        f.write('*FOR_F\n')
        if len(self.FOR6) != 0:
            if (self.FOR6[0][0] == 0 and len(self.FOR6) == 1) or len(self.FOR6) == 0:
                pass
            else:
                for i in range(0,len(self.FOR6)):
                    f.write(str(int(self.FOR6[i][0])) + '  ' )
                    for j in range(1,4):
                        f.write(str(float(self.FOR6[i][j])) + '  ')    
                    f.write('\n')
    ...
    def OnClickCalc(self):
        ...
        Path = self.PATH + r'/bin/DSFrameSolver'
